...
public class WeatherServiceTest extends TestCase{
    ...
    @Test
    public void testCarryUmbrellaMsg() throws Exception{
    	boolean found_msg = false;
    	File sampleFile = new File(WeatherServiceTest.class.getClassLoader()
				 									 .getResource("sampleJsonResponse.json")
				 									 .getFile());
    	String fileContent = readFromFile(sampleFile);
    	...
    	for(NextDayForecast forecast:responseEntity.getForecasts()) {
    		if(WeatherServiceImpl.CARRY_UMBRELLA_MSG.equals(forecast.getAction())) {
    			found_msg = true;
    			break;
    		}
    	}
    	if(!found_msg) {
    		fail("Msg not found: "+WeatherServiceImpl.CARRY_UMBRELLA_MSG);
    	}
    }
